...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # self-attention
        y = self.self_attn(
            self.norm1(x) * (1 + e[1].squeeze(2)) + e[0].squeeze(2), seq_lens, grid_sizes, freqs
        )
        x = x + y * e[2].squeeze(2)
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            ...
            y = self.ffn(self.norm2(x).type_as(x) * (1 + e[4].squeeze(2)) + e[3].squeeze(2))
            x = x + y * e[5].squeeze(2)
            return x
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def _set_gradient_checkpointing(
        self, module=None, value=False, gradient_checkpointing_func=None
    ):
    ...
    def forward(
        ...
        e = self.time_embedding(
            sinusoidal_embedding_1d(self.freq_dim, t).unflatten(0, (bt, seq_len)).type_as(x)
        )
